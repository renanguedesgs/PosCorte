...
namespace PosCorte.API.Controllers
{
    ...
    public class ProjetosController : ControllerBase
    {
        ...
        public async Task<ActionResult<ProjetoDTO>> CriarProjeto([FromBody] CreateProjetoDTO dto)
        {
            ...
            // Seguran�a: arquiteto s� cria projeto em nome pr�prio (admin pode especificar outro).
            if (!IsAdmin && UsuarioId > 0)
                dto.UsuarioId = UsuarioId;

            var usuario = await _usuarioRepo.GetByIdAsync(dto.UsuarioId);
            if (usuario == null)
                return BadRequest(new { error = "Usu�rio n�o encontrado" });
        ...
        /// <summary>
        /// Calcular or�amento de montagem.
        /// Usa f�rmula de Markup Inverso com taxa de 20%.
        /// </summary>
        ...
        /// <summary>Arquiteto abre disputa: congela o escrow at� o admin resolver.</summary>
        ...
        /// <summary>DEV/teste: marca a montagem como conclu�da e inicia a janela de vistoria.</summary>
        ...
        public async Task<IActionResult> SimularConclusaoMontagem(int id, [FromServices] IWebHostEnvironment env)
        {
            if (!env.IsDevelopment())
                return BadRequest(new { error = "Dispon�vel apenas em Development." });

            var resultado = await _vistoriaService.MarcarMontagemConcluidaAsync(id, UsuarioId);
            return MapResultadoVistoria(resultado, "Montagem marcada como conclu�da. Vistoria iniciada.");
        }
        ...
        private IActionResult MapResultadoVistoria(ResultadoVistoria r, string okMsg) => r switch
        {
            ...
            ResultadoVistoria.ProjetoNaoEncontrado => NotFound(new { error = "Projeto n�o encontrado." }),
            ResultadoVistoria.NaoAutorizado => Forbid(),
            ResultadoVistoria.StatusInvalido => BadRequest(new { error = "A��o n�o permitida no status atual do projeto." }),
            ResultadoVistoria.FalhaLiquidacao => BadRequest(new { error = "N�o foi poss�vel liberar o escrow (sem pagamento retido)." }),
            _ => BadRequest(new { error = "Erro desconhecido." })
        };

        /// <summary>Gera cobran�a PIX (Asaas se configurado, sen�o stub para dev).</summary>
        ...
        public async Task<ActionResult<IEnumerable<ProjetoDTO>>> ListarProjetos()
        {
            ...
            // Arquiteto v� apenas os pr�prios projetos; admin v� todos.
            if (!IsAdmin && UsuarioId > 0)
